...
<article>
	...
	<table id="dataTableBody" class="table1"></table>
	...
	for(var i = 0 ; i < dataList.length; i++){
		if(dataList[i].nfix == 'Y'){
			chartHtml += "<tr class='fixed'>"
					...
					+    "<td><a href='detail?nnum=" + dataList[i].nnum + "'>"+ dataList[i].nsubject + "</a></td>"
					+    "<td>" + dataList[i].nwriter + "</td>"
					+    "<td>" + dataList[i].ndate + "</td>"
					+    "<td>" + dataList[i].ncount + "</td>"
					+    "</tr>";
		}
	}
...
function paging(totalData, dataPerPage, pageCount, currentPage) {
	...
	for (var i = first; i <= last; i++) {
		if (currentPage == i) {
			pageHtml += "<li class='page-item'><a href='#' class='page-link active' id='" + i + "'>" + i + "</a></li>";
		} else {
